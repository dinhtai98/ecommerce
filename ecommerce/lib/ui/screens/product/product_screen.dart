        ...
        return ScrollablePositionedList.builder(
          ...
          itemBuilder: (context, index) {
            return _ProductGroup(
              tagName: _viewModel.productInTags[index].name,
              productList: _viewModel.productInTags[index].products,
            );
          },
          itemScrollController: _viewModel
              .getInstanceModel(_instanceId)
...
class _ProductGroup extends StatelessWidget {
  ...
  const _ProductGroup(
      {Key? key, required this.tagName, required this.productList})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return productList.isNotEmpty
        ? SizedBox(
            height: (productList.length * 85).toDouble() + 26,
            child: Column(
              ...
              children: <Widget>[
                ...
                ...productList.map((x) => ProductCard(product: x)).toList()
              ],
            ),
          )
        : const SizedBox.shrink();
